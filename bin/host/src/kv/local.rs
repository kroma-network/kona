...
use kona_client::{
    L1_END_NUMBER_KEY, L1_HEAD_KEY, L2_CHAIN_ID_KEY, L2_CLAIM_BLOCK_NUMBER_KEY, L2_CLAIM_KEY,
...
impl KeyValueStore for LocalKeyValueStore {
    fn get(&self, key: B256) -> Option<Vec<u8>> {
        ...
        match preimage_key.key_value() {
            ...
            L1_END_NUMBER_KEY => Some(self.cfg.l1_end_block_number.to_be_bytes().to_vec()),
    ...
    fn export(&self) -> HashMap<B256, Vec<u8>> {
        ...
        store.insert(
            B256::from(PreimageKey::new_local(L1_END_NUMBER_KEY.to())),
            self.cfg.l1_end_block_number.to_be_bytes().to_vec(),
        );
        store
    }
